...
@section('content')
    <form method="post" action="{{route('admin.products.update', $product)}}" enctype="multipart/form-data">
        ...
        <div class="form-group">
            ...
            <input type="number" class="form-control" value="{{ $product->price }}" step="0.01" placeholder="price" id="price" name="price">
        ...
        <div class="form-group">
            ...
            <select name="category_id" id="category_id">
                @forelse($categories as $category)
                    <option value="{{$category->id}}" @if($category->id === $product->ProductCategory->id) selected @endif>{{$category->name}}</option>
                @empty
